...
module mThick_sheet_side(){
	...
	translate([(calZ_drive_rod_spacing-calVertex_spacing_x)/2,0,(calThick_sheet_side_H-calRod_part_T)/2]){
		difference(){
			...
			cube([2*calVertex_T,calX_guide_rod_spacing,calThick_sheet_side_H-calY_support_to_lower_support_z],center=true);
		}
	}
}
...
module mLower_vertex(){
	difference(){
		union(){
			cube([calVertex_T,calVertex_W,calRod_part_T],center =true);
			translate([0,0,calVertex_outer_L/2+dnaRod_R])	
				cube([calVertex_T,calVertex_W,calVertex_outer_L],center =true);
			...
			translate([0,calVertex_middle_L/2*sin(30),calVertex_middle_L/2*cos(30)+calVertex_outer_L+dnaRod_R-calVertex_middle_W/2*sin(30)])	
				rotate([-30,0,0])
					cube([calVertex_T,calVertex_middle_W,calVertex_middle_L],center =true);
		}
		mLower_vertex_cuts();
	}	
}
...
module mLower_vertex_cuts(){
	color([ 200/255, 0/255, 0/255, 1]){
		...
		translate([0,0,dnaRod_R*2])rotate([90,0,0])
			mHole_horiz(dnaRod_R,100);
			translate([0,(calY_support_to_lower_support_y-dnaRod_R*2*sin(60)),(calY_support_to_lower_support_z-dnaRod_R*2*cos(60))])
					rotate([90-60,0,0])
						mHole_horiz(dnaRod_R,100);
	}
}
